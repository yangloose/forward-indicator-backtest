...
if __name__ == "__main__":
    ...
    st.write("### Monte Carlo Max Drawdown")
    mc_drawdown = monte_carlo_simulation(data)
    st.write(f"Simulated Avg Max Drawdown: {mc_drawdown:.4f}")

    st.write("### Asset Allocation")
    latest_regime = data['Market_Regime'].iloc[-1]
    latest_vol = data['Volatility'].iloc[-1]
    allocation = calculate_allocation(latest_regime, latest_vol)
    st.write(f"Current Regime: {latest_regime}")
    st.write("Suggested Allocation:", allocation)

    st.write("### Indicator Trends")
    plot_forward_score(data)

    st.write("### Download Report")
    pdf_buffer = generate_pdf_report(data, results, allocation)
    st.download_button(
        label="Download PDF Report",
        data=pdf_buffer,
        file_name="AlphaEdge_Macro_Report.pdf",
